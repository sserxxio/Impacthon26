      ...
      <main className="flex-1 pb-12">
        ...
        {loading ? (
          <div className="flex-1 flex items-center justify-center p-20">
            <div className="text-center">
              <div className="animate-spin h-12 w-12 border-4 border-[#ae8d6e] border-t-transparent rounded-full mx-auto mb-4"></div>
              <p className="text-xl">Cargando estadísticas...</p>
            </div>
          </div>
        ) : !hotelId || !summary ? (
          <div className="p-8 text-center text-[#ae8d6e]">
            No se han podido cargar los datos del hotel.
          </div>
        ) : (
          <div className="flex flex-col">
            {error && (
              <div className="mx-8 bg-[#c50000]/20 border border-[#c50000] text-[#c50000] p-4 rounded-xl mb-6">
                ⚠️ {error}
              </div>
            )}
            ...
            <div className="mx-8 grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              <div className="bg-white border border-[#ae8d6e]/30 p-6 rounded-xl shadow-md">
            ...
            <div className="mx-8 space-y-8 mb-8">
              ...
              <div className="bg-white border border-[#ae8d6e]/30 p-6 rounded-2xl shadow-md">
                <h3 className="text-xl font-bold text-[#683110] mb-6">
                    ...
                    <Tooltip
                      content={({ active, payload, label }) => {
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-white border border-[#ae8d6e]/40 p-4 rounded-xl shadow-2xl backdrop-blur-md">
                              <p className="text-[#5e0710] font-bold mb-2">{label}</p>
                              ...
                              {sStarts.length > 0 && (
                                <div className="mt-2 pt-2 border-t border-[#ae8d6e]/30">
                                  {sStarts.map((s: string, i: number) => (
                                    <p key={i} className="text-[#683110] font-bold text-sm flex items-center gap-2">
                                      <span>🚀</span> {s}
                                    </p>
                                  ))}
                                </div>
                              ...
                              {sEnds.length > 0 && (
                                <div className="mt-2 pt-2 border-t border-[#ae8d6e]/30">
                                  {sEnds.map((s: string, i: number) => (
                                    <p key={i} className="text-[#c50000] font-bold text-sm flex items-center gap-2">
                    ...
                    <Tooltip
                      content={({ active, payload, label }) => {
                        if (active && payload && payload.length) {
                          ...
                          const sEnds = payload[0].payload.completedStrategiesArr || [];
                          return (
                            <div className="bg-white border border-[#ae8d6e]/30 p-4 rounded-xl shadow-2xl backdrop-blur-md">
                              <p className="text-[#ae8d6e] font-bold mb-2">{label}</p>
                              {payload.map((entry: any, index: number) => (
                                <p key={index} style={{ color: entry.color }} className="text-sm flex justify-between gap-4">
                                  <span>{entry.name}:</span>
                                  <span className="font-mono">
              ...
              <div className="bg-white border border-[#ae8d6e]/30 p-6 rounded-2xl">
                <h3 className="text-xl font-bold text-[#ae8d6e] mb-6">
                  📈 ROI vs Utilidad
                </h3>
                <ResponsiveContainer width="100%" height={300}>
              ...
              <div className="bg-white border border-[#ae8d6e]/30 p-6 rounded-2xl">
                <h3 className="text-xl font-bold text-[#c50000] mb-6">
            ...
            {analysis ? (
              <div className="mx-8 mb-8">
                <h2 className="text-3xl font-bold text-[#683110] mb-6">
                  🤖 Análisis IA Generado
                </h2>

                {analysis.tendencia_general && (
                  <div className="bg-white border border-[#ae8d6e]/50 p-6 rounded-2xl mb-6">
                    <h3 className="text-[#683110] font-bold mb-3">📊 Tendencia General</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  {analysis.fortalezas && analysis.fortalezas.length > 0 && (
                    <div className="bg-transparent border border-[#ae8d6e]/30 p-6 rounded-2xl">
                      <h3 className="text-[#683110] font-bold mb-3">💪 Fortalezas</h3>
                      <ul className="space-y-2">
                        {analysis.fortalezas.map((f, i) => (
                          <li key={i} className="text-[#5e0710] flex items-start gap-2">
                            <span className="text-[#683110] mt-1">✓</span>
                            {f}
                          </li>
                        ))}
                      </ul>
                  ...
                  {analysis.debilidades && analysis.debilidades.length > 0 && (
                    <div className="bg-transparent border border-[#ae8d6e]/30 p-6 rounded-2xl">
                      <h3 className="text-[#c50000] font-bold mb-3">⚠️ Debilidades</h3>
                      <ul className="space-y-2">
                        {analysis.debilidades.map((d, i) => (
                          <li key={i} className="text-[#5e0710] flex items-start gap-2">
                            <span className="text-[#c50000] mt-1">×</span>
                            {d}
                          </li>
                        ))}
                      </ul>
                  ...
                  {analysis.oportunidades && analysis.oportunidades.length > 0 && (
                    <div className="bg-transparent border border-[#ae8d6e]/30 p-6 rounded-2xl">
                      <h3 className="text-[#683110] font-bold mb-3">🎯 Oportunidades</h3>
                      <ul className="space-y-2">
                  ...
                  {analysis.amenazas && analysis.amenazas.length > 0 && (
                    <div className="bg-transparent border border-[#ae8d6e]/30 p-6 rounded-2xl">
                      <h3 className="text-[#ae8d6e] font-bold mb-3">⚡ Amenazas</h3>
                      <ul className="space-y-2">
                        {analysis.amenazas.map((a, i) => (
                          <li key={i} className="text-[#5e0710] flex items-start gap-2">
                            <span className="text-[#ae8d6e] mt-1">!</span>
                            {a}
                          </li>
                        ))}
                      </ul>
                ...
                {analysis.recomendaciones_clave &&
                  analysis.recomendaciones_clave.length > 0 && (
                    <div className="bg-white border border-[#ae8d6e]/30 p-6 rounded-2xl mb-6">
                      <h3 className="text-[#683110] font-bold mb-4">
                      ...
                      <div className="space-y-4">
                        {analysis.recomendaciones_clave.map((rec, i) => (
                          <div
                            key={i}
                            className="bg-transparent p-4 rounded-xl border border-[#ae8d6e]/30"
                          >
                            <h4 className="text-[#683110] font-semibold mb-2">
                              {i + 1}. {rec.titulo}
                            </h4>
                            <p className="text-[#5e0710] text-sm mb-2">
                              {rec.descripcion}
                            </p>
                            <p className="text-[#683110] text-sm font-semibold">
              ...
              <div className="mx-8 bg-white p-8 rounded-2xl border border-[#ae8d6e]/50 text-center">
                <div className="inline-block animate-spin h-8 w-8 border-4 border-[#ae8d6e] border-t-transparent rounded-full mb-4"></div>
                <p className="text-[#683110] font-bold">Generando análisis con IA...</p>
              </div>
            ) : null}

            {/* Mejor Mes */}
            {summary.bestMonth && (
              <div className="mx-8 bg-[#c50000] border border-[#ae8d6e]/30 p-8 rounded-2xl mb-8">
                <h3 className="text-2xl font-bold text-[#f5f4f1] mb-4">
                  🏆 Mejor Mes: {monthNames[summary.bestMonth.month - 1]}{" "}
                  {summary.bestMonth.year}
                </h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    <p className="text-[#ae8d6e] text-sm mb-1">Ingresos</p>
                    <p className="text-2xl font-bold text-[#f5f4f1]">
                      €{summary.bestMonth.ingresos.toLocaleString("es-ES")}
                    </p>
                  </div>
                  <div>
                    <p className="text-[#ae8d6e] text-sm mb-1">Utilidad</p>
                    <p className="text-2xl font-bold text-[#f5f4f1]">
                      €{summary.bestMonth.utilidad.toLocaleString("es-ES")}
                    </p>
                  </div>
                  <div>
                    <p className="text-[#ae8d6e] text-sm mb-1">Ocupación</p>
                    <p className="text-2xl font-bold text-[#f5f4f1]">
                      {summary.bestMonth.ocupacion}%
                    </p>
                  </div>
                  <div>
                    <p className="text-[#ae8d6e] text-sm mb-1">ROI</p>
                    <p className="text-2xl font-bold text-[#f5f4f1]">
        ...
        <div className="mx-8 flex justify-center mb-8">
          <button
            ...
            className="bg-transparent border-2 border-[#683110] text-[#683110] hover:bg-[#683110] hover:text-[#f5f4f1] transition-all px-8 py-4 rounded-2xl font-bold uppercase tracking-widest text-sm shadow-sm"
          >
            ✏️ Editar Amenidades
          </button>
        </div>
